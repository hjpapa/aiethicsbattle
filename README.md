...
`OPENAI_API_KEY`는 서버 라우트에서만 사용됩니다. `SUPABASE_SECRET_KEY`와 `SUPABASE_SERVICE_ROLE_KEY`도 서버 전용이며 `NEXT_PUBLIC_` 접두사를 붙이면 안 됩니다.
...
이 MVP는 학생 이름, 학교명, 주소, 연락처를 저장하지 않는 익명 세션 구조입니다. 공개 키에는 익명 토론 데이터의 `INSERT`만 허용하고 `SELECT` 정책은 두지 않습니다. 서버 비밀 키를 설정하면 같은 서버 라우트가 해당 키를 우선 사용합니다.
